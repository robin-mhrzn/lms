...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  return (
    <MainContainer>
      <div
        className="max-w-6xl mx-auto space-y-8 mb-6 p-6 rounded-lg shadow-lg"
        style={{
        ...
        <div className="lg:col-span-3">
          ...
          <div className="space-y-6 mb-6">
            {course.additionalType
              ...
              .map((type, index) => (
                <div className="space-y-6 mb-6" key={`additionalType_${index}`}>
              ...
              <div className="space-y-4">
                {course.modules.map((lessonPlan, idx) => (
                  <div key={`module_${idx}`}>
                    ...
                    {idx < course.modules.length - 1 && <hr className="mt-3" />}
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <div className="sticky top-4 space-y-6">
            <Card className="w-full max-w-sm shadow-xl rounded-2xl">
              <div className="bg-white p-6">
                ...
                <Button className="w-full mb-4">Purchase this course</Button>

                <hr className="mb-4" />
                {course.additionalType
                  .filter((type) => type.additionalType === "Course Contents")
                  .map((type, index) => (
